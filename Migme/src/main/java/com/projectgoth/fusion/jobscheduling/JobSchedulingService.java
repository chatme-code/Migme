...
public class JobSchedulingService extends Application {
   private static Logger log = LoggerFactory.getLogger(JobSchedulingService.class);
   ...
   private void configureServant(Properties properties) throws Exception {
      ...
      if (unsProxy == null) {
         log.error("Aborting, weren't able to find a UNS proxy..");
         System.exit(1);
      }
   ...
   public int run(String[] arg0) {
      ...
      try {
         ...
         if (interrupted()) {
            log.error("JobSchedulingService " + this.hostName + ": terminating");
