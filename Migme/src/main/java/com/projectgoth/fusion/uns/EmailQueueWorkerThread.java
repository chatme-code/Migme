...
public class EmailQueueWorkerThread extends Thread {
   private static final Logger log = LoggerFactory.getLogger(ConfigUtils.getLoggerName(EmailQueueWorkerThread.class));
   private static final Logger emailLogger = LoggerFactory.getLogger("EmailQueueWorker");
   ...
   private void newEmailSendingLogic() throws Exception {
      ...
      if (isSystemNotification && note.getMimeType() == null) {
         ...
         if (log.isDebugEnabled()) {
            try {
               Address[] recipients = message.getRecipients(jakarta.mail.Message.RecipientType.TO);
