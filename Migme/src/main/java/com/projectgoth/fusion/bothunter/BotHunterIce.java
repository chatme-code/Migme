...
public class BotHunterIce extends Application {
   private static final Logger log = LoggerFactory.getLogger(ConfigUtils.getLoggerName(BotHunterIce.class));
   ...
   private void getSettings(String[] args) {
      ...
      if (this.booleanIceProperty(p, "VerboseAnalysis")) {
         monitorLogger = LoggerFactory.getLogger(ConfigUtils.getLoggerName(AnalysisThread.class));
//          monitorLogger.setLevel(Level.DEBUG); // SLF4J: removed
      }

      if (this.booleanIceProperty(p, "VerboseKick")) {
         monitorLogger = LoggerFactory.getLogger(ConfigUtils.getLoggerName(KickThread.class));
//          monitorLogger.setLevel(Level.DEBUG); // SLF4J: removed
      }

      if (this.booleanIceProperty(p, "VerbosePacketCapture")) {
         monitorLogger = LoggerFactory.getLogger(ConfigUtils.getLoggerName(MonitorThread.class));
//          monitorLogger.setLevel(Level.DEBUG); // SLF4J: removed
      }
