...
public abstract class LazyLoader<TValueType> {
   ...
   public LazyLoader(String logCategorySubName, long localCacheTTLMillies) {
      ...
      if (StringUtil.isBlank(logCategorySubName)) {
         this.logger = LoggerFactory.getLogger(ConfigUtils.getLoggerName(this.getClass()));
      } else {
         this.logger = LoggerFactory.getLogger(ConfigUtils.getLoggerName(LazyLoader.class) + "[" + logCategorySubName + "]");
      }
