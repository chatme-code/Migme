...
import net.spy.memcached.SockIOPool;
import java.io.File;
...
public class MemCachedUtils {
   private static final Logger log = LoggerFactory.getLogger(ConfigUtils.getLoggerName(MemCachedUtils.class));
   ...
   public static MemCachedClient getMemCachedClient(MemCachedUtils.Instance pool) {
      synchronized(pool) {
         ...
         try {
            ...
            log.error("Unable to get client:" + pool + " from map");
         }
   ...
   private static boolean loadProperties() {
      ...
      try {
         ...
         log.error("Failed to load memcached.properties, it should be at [" + propertiesLocation + "], aborting.", var2);
      }
