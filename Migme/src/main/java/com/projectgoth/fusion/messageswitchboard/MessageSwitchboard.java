...
public class MessageSwitchboard extends Application {
   ...
   public int run(String[] arg0) {
      ...
      try {
         ...
         if (interrupted()) {
            log.error("MessageSwitchboard " + this.hostName + ": terminating");
   ...
   private RegistryPrx getRegistryPrx() {
      ...
      try {
         ...
         log.error("Connection to [" + registryPrx + "] failed. ", var5);
         return null;
      }

      if (registryPrx == null) {
         log.error("Connection to [" + registryPrx + "] failed");
         return null;
      } else {
         return registryPrx;
      }
   ...
   static {
      log = new LogFilter(LoggerFactory.getLogger(MessageSwitchboard.class), SystemPropertyEntities.ChatSyncSettings.LOG_EXCLUSION_FILTER);
