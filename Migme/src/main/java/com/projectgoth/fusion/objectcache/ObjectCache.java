...
public class ObjectCache extends Application {
   ...
   private static final Logger log = LoggerFactory.getLogger(ConfigUtils.getLoggerName(ObjectCache.class));
   ...
   public int run(String[] arg0) {
      ...
      try {
         ...
         log.error("Object Cache " + hostName + ": Failed to initialize MogileFS. Exception: " + var12.toString());
         return 1;
      }
      ...
      try {
         ...
         log.error("Object Cache " + hostName + ": Connection to [" + registryPrx + "] failed. ", var11);
         return 1;
      }

      if (registryPrx == null) {
         log.error("Object Cache " + hostName + ": Connection to [" + registryPrx + "] failed");
         return 1;
      } else {
         ...
         try {
            ...
            log.error("Object Cache " + hostName + ": Connection to a SessionCache failed.", var10);
            return 1;
         }

         if (sessionCachePrx == null) {
            log.error("Object Cache " + hostName + ": Connection to a SessionCache failed, the session cache proxy is null.");
            return 1;
         } else {
            if (logMessagesToFile) {
               ...
               try {
                  ...
                  log.error("Object Cache " + hostName + ": Connection to a MessageLogger failed. Exception: " + var9.toString());
                  return 1;
               }

               if (messageLoggerPrx == null) {
                  log.error("Object Cache " + hostName + ": Connection to a MessageLogger failed");
                  return 1;
               }
            }
            ...
            try {
               ...
               log.error("Object Cache " + hostName + ": Failed to initialize data grid. Exception: " + var14.toString());
               return 1;
            }
            ...
            if (interrupted()) {
               log.error("Application was interrupted, shutting down");
            } else {
