...
public abstract class AbstractDatabaseMaintenance {
   protected static Logger log = LoggerFactory.getLogger(ConfigUtils.getLoggerName(AbstractDatabaseMaintenance.class));
   ...
   protected void configureDataSourceDefaults(ComboPooledDataSource dataSource) {
      try {
         ...
         log.error("failed to setup driver class", var3);
         System.exit(1);
      }
   ...
   protected DataSource configureMisDataSource() {
      ...
      try {
         ...
         log.error("failed to setup mis datasource", var3);
         System.exit(1);
         return null;
      }
   }
   ...
   protected DataSource configureMasterDataSource() {
      ...
      try {
         ...
         log.error("failed to setup master datasource", var3);
         System.exit(1);
         return null;
      }
   }
   ...
   protected DataSource configureSlaveDataSource() {
      ...
      try {
         ...
         log.error("failed to setup slave datasource", var3);
         System.exit(1);
         return null;
      }
   }
